...
const Dashboard = () => {
  ...
  const DataAdmin = location.state.fetchDataAdmin;
  ...
  const setPemilihan = async () => {
    if (!DataAdmin.statusPemilihan) {
      Swal.fire({
        icon: "question",
        text: "Ingin Memulai Pemilihan ?",
        showCancelButton: true,
        confirmButtonText: "Ya",
      }).then(async (result) => {
        if (result.isConfirmed) {
          await contract.methods
            .setPemilihan(!DataAdmin.statusPemilihan)
            .send({ from: accounts[0] });
          Swal.fire({
            icon: "success",
            text: "Pemilihan Sudah Dimulai 🫡",
          }).then(async () => {
      ...
      Swal.fire({
        icon: "question",
        text: "Ingin Menghentikan Pemilihan ?",
        showCancelButton: true,
        confirmButtonText: "Ya",
      }).then(async (result) => {
        if (result.isConfirmed) {
                          ...
                          <li className="list-group-item">
                            <div className="row text-center">
                              <div className="col-12">
                                {!DataAdmin.statusPemilihan ? (
                                  <button
                                    type="button"
                                    class="btn bg-green shadow text-white"
                                    onClick={setPemilihan}
                                  >
                                    Mulai Pemilihan
                                  </button>
                                ) : (
